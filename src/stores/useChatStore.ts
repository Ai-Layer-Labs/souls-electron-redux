...
export interface IChatStore {
  ...
  updateCanvasData: (chatId: string, canvasData: string | null) => Promise<boolean>;
...
const useChatStore = create<IChatStore>((set, get) => ({
  ...
  editChat: (chat: Partial<IChat>) => {
    ...
    if (isValidTemperature(chat.temperature)) {
  ...
  createChat: async (
    ...
    const ok = await window.electron.db.run(
      `INSERT INTO chats (id, summary, model, systemMessage, temperature, maxCtxMessages, maxTokens, stream, prompt, input, folderId, createdAt, canvasData)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        $chat.id,
        $chat.summary,
  ...
  updateChat: async (chat: { id: string } & Partial<IChat>) => {
    ...
    if (isValidTemperature(chat.temperature)) {
      $chat.temperature = chat.temperature;
      params.push($chat.temperature || 0);
    }
    if (isNumber(chat.maxTokens) && chat.maxTokens > 0) {
      stats.push('maxTokens = ?');
      $chat.maxTokens = chat.maxTokens;
    ...
    if (!isUndefined(chat.canvasData)) {
      stats.push('canvasData = ?');
      $chat.canvasData = chat.canvasData;
      params.push($chat.canvasData);
    }
  ...
  updateCanvasData: async (chatId: string, canvasData: string | null) => {
    if (chatId === tempChatId) {
      // Don't persist canvas data for temp chat directly, maybe handle in component state?
      // Or potentially update tempStage if needed, but DB update is irrelevant.
      return false;
    }
    try {
      await window.electron.db.run(
        `UPDATE chats SET canvasData = ? WHERE id = ?`,
        [canvasData, chatId],
      );
      set(
        produce((state: IChatStore) => {
          if (chatId === state.chat.id) {
            state.chat.canvasData = canvasData;
          }
          const chatIndex = state.chats.findIndex((c) => c.id === chatId);
          if (chatIndex > -1) {
            state.chats[chatIndex].canvasData = canvasData;
          }
        }),
      );
      // debug('Updated canvasData for chat:', chatId);
      return true;
    } catch (error) {
      captureException(error instanceof Error ? error : new Error(String(error)));
      console.error('Failed to update canvas data:', error);
      return false;
    }
  },
  getChat: async (id: string) => {
    const chat = (await window.electron.db.get(
      'SELECT id, summary, model, systemMessage, maxTokens, temperature, context, maxCtxMessages, stream, prompt, input, folderId, createdAt, canvasData FROM chats where id = ?',
      id,
    )) as IChat;
    if (chat) {
      try {
        chat.prompt = chat.prompt ? JSON.parse(chat.prompt as unknown as string) : null;
      } catch (err: any) {
        captureException(err);
      }
    }
  ...
  fetchChat: async (limit: number = 300, offset = 0) => {
    let rows: IChat[] | undefined;
    try {
        rows = (await window.electron.db.all(
          'SELECT id, summary, folderId, createdAt, canvasData FROM chats ORDER BY createdAt DESC limit ? offset ?',
          [limit, offset],
        )) as IChat[] | undefined; // Explicitly allow undefined
    } catch (error) {
        console.error('Failed to fetch chats from DB:', error);
        captureException(error instanceof Error ? error : new Error(String(error)));
        rows = undefined; // Ensure rows is undefined on error
    }

    // Check if rows is an array before mapping
    const chats = Array.isArray(rows) ? rows.map((chat) => {
      // Note: Parsing chat.prompt is handled in getChat, removed from here.
      return chat;
    }) : []; // Default to empty array if rows is not an array
      ...
      produce((state: IChatStore) => {
        ...
        if (msg) {
          if (bookmarkId !== undefined) {
            msg.bookmarkId = bookmarkId === null ? undefined : bookmarkId;
          }
        }
      }),
    );
