    ...
    try {
      ...
      if (!hasSystemMessage) {
        ...
        if (systemPrompt) {
          ...
          history = [new SystemMessage(`Default working language: English
...
6. Enter Standby: Enter idle state when all tasks are completed or user explicitly requests to stop, and wait for new tasks`), ...history];
        }
      }
